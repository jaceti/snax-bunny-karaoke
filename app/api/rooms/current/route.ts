...
// is the same one printed into every singer QR code.
export async function GET() {
  try {
    const db = (env as unknown as { DB?: D1Database }).DB;
    if (!db) return Response.json({ error: "The room database isn’t connected yet." }, { status: 503 });
    const current = await db.prepare("SELECT c.code, c.invite_token, r.requests_open, r.ends_at FROM current_room c JOIN rooms r ON r.code = c.code WHERE c.id = 1").first<{ code: string; invite_token: string; requests_open: number; ends_at: string | null }>();
    ...
    return Response.json({ code: current.code, inviteToken: current.invite_token }, { headers: { "cache-control": "no-store" } });
  } catch (error) {
