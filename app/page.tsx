...
export default function Home(){
  ...
  async function runSearch(term:string){
    ...
    try{const response=await fetch(`/api/search?q=${encodeURIComponent(term)}`,{headers:{"x-room-code":roomCode,"x-room-invite":inviteRef.current}});const data=await response.json() as {results?:Song[];error?:string};if(!response.ok)throw new Error(data.error||"Search took a mic break.");
      ...
      if(!incoming.length)setNotice("No suitable karaoke tracks found. Try adding the artist name.");}
  ...
  return <main className={`snax-shell view-${screen}`}>
    ...
    {screen==="singer"&&<section className="singer-stage"><header className="app-header"><button className="wordmark" onClick={home}>SNAX</button><span>Room <strong>{roomCode}</strong></span><span className="singer-chip">{singerName}</span></header><div className="singer-grid"><div className="search-panel"><p className="eyebrow">You’re in, {singerName}</p><p className="singer-memory-note">Your name stays the same on this browser until 3 AM Pacific.</p><h1>Pick your song</h1>{room&&!room.requestsOpen&&<p className="requests-closed">Requests are closed for tonight. The bunny is tired.</p>}{(!room||room.requestsOpen)&&<><form className="song-search" onSubmit={search}><label htmlFor="song">Search a song or artist</label><div><input id="song" value={query} onChange={event=>setQuery(event.target.value)} placeholder="Robyn, Chappell Roan, ABBA…"/><button disabled={searching}>{searching?"Searching…":"Find karaoke"}</button></div><small>Karaoke with lyrics preferred. Labeled lead/guide-vocal versions excluded.</small></form><div className="results">{results.slice(0,shown).map(song=><article key={song.videoId}><img src={song.thumbnail} alt=""/><div><strong><a href={`https://www.youtube.com/watch?v=${song.videoId}`} target="_blank" rel="noreferrer" title="Open on YouTube">{song.title}</a></strong><small>{song.channel}</small><button onClick={()=>void addSong(song)} disabled={busy}>Add to lineup +</button></div></article>)}</div>{results.length>shown&&<button type="button" className="load-more" onClick={()=>setShown(count=>count+PAGE_SIZE)}>Show more ({results.length-shown} left) ↓</button>}{results.length>0&&results.length<=shown&&<p className="results-end">That’s every match for this search. Try adding the artist or a word from the title for more.</p>}</>}</div><QueuePanel room={room} busy={busy} onControl={control}/></div></section>}
