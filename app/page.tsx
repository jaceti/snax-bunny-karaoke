...
function TipCard(){
  ...
  return <section className="tip-card"><p className="eyebrow">Tip the bunny</p><h2>Tip Snax on Venmo</h2><p>Tips keep the karaoke coming. @{VENMO_HANDLE}</p><div className="tip-amounts">{[5,10,20].map(value=><button key={value} type="button" className={amount===value?"selected":""} onClick={()=>setAmount(value)}>${value}</button>)}</div><button type="button" className="tip-go" onClick={()=>tip(amount)}>{amount?`Tip $${amount} on Venmo`:"Open Venmo"} <span>→</span></button></section>;
}
