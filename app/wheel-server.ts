...
export async function readWheel(db:D1Database,code:string,now=Date.now()):Promise<WheelState|null>{
  ...
  if(wheel.phase==="spinning"&&wheel.endsAt!==null&&now>=wheel.endsAt)wheel.phase="winner";
  return wheel;
}
...
export async function openWheel(db:D1Database,code:string){
  ...
  const wheel:WheelState={id:crypto.randomUUID(),phase:"ready",entries:roster,winnerIndex:null,startedAt:null,endsAt:null,rotation:0};
  await db.batch([
...
export async function spinWheel(db:D1Database,code:string,wheelId:string,now=Date.now()){
  ...
  await db.batch([
    ...
    db.prepare("UPDATE queue_items SET sort_order=(SELECT COALESCE(MIN(sort_order),0)-1 FROM queue_items WHERE room_code=? AND status='pending') WHERE id=? AND room_code=? AND status='pending' AND EXISTS (SELECT 1 FROM room_wheel WHERE room_code=? AND state=?)").bind(code,winner.queueId,code,code,serialized),
  ]);
}
export async function closeWheel(db:D1Database,code:string,wheelId:string,now=Date.now()){
  ...
  await db.batch([
    db.prepare("UPDATE rooms SET playback_status=CASE WHEN NOT EXISTS (SELECT 1 FROM queue_items WHERE room_code=? AND status='playing') THEN 'idle' WHEN (SELECT resume_playback FROM room_wheel WHERE room_code=?)=1 THEN 'playing' ELSE 'paused' END WHERE code=? AND EXISTS (SELECT 1 FROM room_wheel WHERE room_code=? AND state=?)").bind(code,code,code,code,row.state),
